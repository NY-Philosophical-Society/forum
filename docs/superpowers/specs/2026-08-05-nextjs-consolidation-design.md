...
durable object storage. Production uses the public `forum-images` bucket in the
same Supabase project as Auth and Postgres; server-side writes and deletes use
the Supabase secret key. A production or Preview deployment must therefore fail
startup validation when `STORAGE_PROVIDER=local`; configuring Supabase Storage
is a **deployment prerequisite for this consolidation**, not an
...
`DIRECT_DATABASE_URL` is direct for migrations. Production adds `connection_limit=1` and
...
`DATABASE_URL`, `DIRECT_DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SECRET_KEY`,
...
- The Supabase Storage provider and `forum-images` bucket must be configured. Preview and
