...
- **Profiles and photos.** The founding requirement was a real name *and* a photo. Every author
  ...
  (`apps/web/src/server/storage-provider.ts`): a zero-credential local-disk stub in dev and the
  hosted Supabase Storage bucket in production. Account management (change/set password, change email, JSON data
...
3. **Image storage bucket.** Avatars and post-image embeds are stored via the
   ...
   production. In the hosted Supabase project, create a public Files bucket named
   `forum-images`, restrict it to 5 MB JPEG files, and set
   `STORAGE_PROVIDER=supabase` plus `SUPABASE_STORAGE_BUCKET=forum-images`. Server-side
   writes and deletes use `SUPABASE_SECRET_KEY`; do not expose that key to clients.
4. **Hosting.** The database is already Postgres on Supabase; point `DATABASE_URL` at the hosted
   ...
   `?pgbouncer=true&connection_limit=1`) and `DIRECT_DATABASE_URL` at the direct one on 5432 — Prisma runs
   migrations over the direct URL, and aiming migrations at the pooled one is the classic way to
   break this. Set `SUPABASE_SECRET_KEY` to the hosted project's secret; server code reuses
   `NEXT_PUBLIC_SUPABASE_URL` for the project URL unless `SUPABASE_URL` overrides it. Set the
