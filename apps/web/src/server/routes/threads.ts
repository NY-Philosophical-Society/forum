...
import { optionalAuth, requireAdmin, requireAuth, requireVerified } from "../guards";
...
import { softDeleteThread } from "../moderation";
import { notify, toSnippet } from "../notifications";
import { canViewThread, isActiveChapterMember } from "../chapter-access";
...
threadsRouter.get("/", optionalAuth, async (req, res) => {
  ...
  const [threads, total] = await Promise.all([
    prisma.thread.findMany({
      ...
      // Pins ride in front of either sort as a separate column, so unpinned
      // threads fall through to their real order and hotScore is never touched
      // by a pin. `nulls: "last"` is load-bearing and must stay explicit:
